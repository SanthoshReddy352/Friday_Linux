...
# Download Piper Engine for Windows
if (-Not (Test-Path "piper\piper.exe")) {
    Write-Host "Downloading Piper TTS Engine..."
    Invoke-WebRequest -Uri "https://github.com/rhasspy/piper/releases/download/v1.2.0/piper_windows_amd64.zip" -OutFile "piper_windows_amd64.zip"
    Expand-Archive -Path "piper_windows_amd64.zip" -DestinationPath "piper" -Force
    # The zip contains a folder piper/, move contents up or adjust path
    if (Test-Path "piper\piper\piper.exe") {
        Move-Item -Path "piper\piper\*" -Destination "piper\" -Force
        Remove-Item "piper\piper" -Recurse -Force
    }
...
if (-Not (Test-Path "models\en_US-lessac-medium.onnx")) {
    Write-Host "Downloading Piper Voice Model..." -ForegroundColor Cyan
    Invoke-WebRequest -Uri "https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx?download=true" -OutFile "models\en_US-lessac-medium.onnx"
} else {
    Write-Host "Piper Voice Model already exists."
}

# 4. Piper Voice Config
if (-Not (Test-Path "models\en_US-lessac-medium.onnx.json")) {
    Write-Host "Downloading Piper Voice Config..." -ForegroundColor Cyan
    Invoke-WebRequest -Uri "https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json?download=true" -OutFile "models\en_US-lessac-medium.onnx.json"
} else {
    Write-Host "Piper Voice Config already exists."
}
